...
const chooseVenue = (feeds, direction) => {
    if (direction === 'long') {
...
class ExecutionEngine {
    ...
    execute(order, snapshot) {
        ...
        const constrainedNoise = Math.max(-this.config.maxSlippageBps / 10000, Math.min(this.config.maxSlippageBps / 10000, baseNoise / snapshot.consolidatedOHLCV.close));
        const fillPrice = venue.price * (1 + constrainedNoise * directionFactor);
        const primarySize = order.notional / fillPrice;
        const latencyMs = this.config.latencyRange.min +
