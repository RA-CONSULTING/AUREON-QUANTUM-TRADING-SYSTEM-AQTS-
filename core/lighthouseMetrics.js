...
exports.computeAnomalyPointer = computeAnomalyPointer;
exports.computeEffectiveGravity = computeEffectiveGravity;
exports.computeLinearCoherence = computeLinearCoherence;
exports.computeNonlinearCoherence = computeNonlinearCoherence;
exports.computeLighthouseIntensity = computeLighthouseIntensity;
exports.computeLighthouseMetrics = computeLighthouseMetrics;
exports.computeHarmonicLoopMetrics = computeHarmonicLoopMetrics;
/**
 * PHI — Golden ratio
 */
 ...
 */
function computeAnomalyPointer(snapshot, priceHistory, volumeHistory) {
    ...
    if (recentPrices.length < 3)
        return volumeSpike * 0.5 + spreadAnomaly * 0.5;
    const diffs = [];
 ...
 */
function computeEffectiveGravity(priceHistory, timeHistory, alpha = 1.0, tolerance = 0.1) {
    if (priceHistory.length < 5 || timeHistory.length < 5)
        return 0;
 ...
 */
function computeLinearCoherence(priceHistory, window = 20) {
 ...
 */
function computeNonlinearCoherence(priceHistory, window = 20) {
    if (priceHistory.length < window)
        return 0;
 ...
 * - C_lin: 1.0 (baseline)
 */
function computeLighthouseIntensity(C_lin, C_nonlin, G_eff, Q, weights = [1.0, 1.2, 1.2, 0.8]) {
    const metrics = [C_lin, C_nonlin, G_eff, Q];
    // Geometric mean with weights
    let product = 1.0;
    for (let i = 0; i < metrics.length; i++) {
        if (metrics[i] <= 0)
            return 0; // any zero kills consensus
 ...
 */
function computeLighthouseMetrics(snapshot, priceHistory, volumeHistory, timeHistory) {
 ...
 */
function computeHarmonicLoopMetrics(lambdaSeries, maskWidth = 10) {
